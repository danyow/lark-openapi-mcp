...
export const sheetsV3SpreadsheetSheetFilterViewConditionCreate = {
  ...
  httpMethod: 'POST',
  description:
    '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-创建筛选条件-在筛选视图的指定列创建筛选条件，包括筛选的类型、比较类型、筛选参数等',
  accessTokens: ['tenant', 'user'],
  schema: {
    data: z
      .object({
        ...
        filter_type: z
          ...
          .optional(),
        compare_type: z.string().describe('比较类型。了解更多，参考[筛选条件指南]').optional(),
        expected: z.array(z.string()).describe('筛选参数。了解更多，参考[筛选条件指南]').optional(),
      })
      .optional(),
    path: z
      .object({
        spreadsheet_token: z
          ...
          .optional(),
        sheet_id: z.string().describe('工作表 ID，通过[获取工作表] 获取').optional(),
        filter_view_id: z.string().describe('筛选视图 ID。通过[查询筛选视图]获取').optional(),
      })
      .optional(),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
...
export const sheetsV3SpreadsheetSheetFilterViewConditionGet = {
  ...
  httpMethod: 'GET',
  description:
    '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-获取筛选条件-获取筛选视图某列的筛选条件，包括筛选的类型、比较类型、筛选参数等',
  accessTokens: ['tenant', 'user'],
  schema: {
    path: z
      .object({
...
export const sheetsV3SpreadsheetSheetFilterViewConditionQuery = {
  ...
  httpMethod: 'GET',
  description:
    '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-查询筛选条件-查询指定筛选视图的所有筛选条件，包括筛选的类型、比较类型、筛选参数等',
  accessTokens: ['tenant', 'user'],
  schema: {
    path: z
      .object({
...
export const sheetsV3SpreadsheetSheetFilterViewConditionUpdate = {
  ...
  httpMethod: 'PUT',
  description:
    '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-更新筛选条件-更新筛选视图指定列的筛选条件，包括筛选的类型、比较类型、筛选参数等',
  accessTokens: ['tenant', 'user'],
  schema: {
    data: z
      .object({
      ...
      condition: z
        .object({
          ...
          compare_type: z
            ...
            .optional(),
          expected: z.array(z.string()).describe('筛选参数。不同筛选类型的筛选参数限制不同，详情参考[筛选指南]'),
        })
        .describe('设置筛选条件'),
    }),
    path: z
    ...
    path: z
      .object({
        spreadsheet_token: z
          ...
          .optional(),
        sheet_id: z.string().describe('电子表格工作表的 ID。调用[获取工作表]获取 ID').optional(),
      })
      .optional(),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
    ...
    path: z
      .object({
        spreadsheet_token: z
          ...
          .optional(),
        sheet_id: z.string().describe('电子表格工作表的 ID。调用[获取工作表]获取 ID').optional(),
        float_image_id: z.string().describe('工作表内浮动图片的唯一标识。通过[查询浮动图片]接口获取').optional(),
      })
      .optional(),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
...
export const sheetsV3SpreadsheetSheetGet = {
  ...
  schema: {
    path: z.object({
      ...
      sheet_id: z.string().describe('工作表的 ID。调用[获取工作表]获取 ID'),
    }),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
};
    ...
    path: z
      .object({
        spreadsheet_token: z
          ...
          .optional(),
        sheet_id: z.string().describe('工作表的 ID。调用[获取工作表]获取 ID').optional(),
      })
      .optional(),
    useUAT: z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
  },
